...
select group_name, courses.course_name from courses inner join groups g on g.id = courses.groups_id;
--4. Выведите курсы групп у которых курс начался с 202011 по 202333
select course_name from courses where  start_date between '2020-01-01' and '2023-03-03';
--5. Выведите имена, дату рождения студентов , которые родились с 198011 по 20041212, и названиегруппы
select first_name,students.date_of_birth from students where date_of_birth between '1980-01-01' and '2004-12-12' order by group_id;
-- 6. Вывести полное имя, возраст, почту студентов и название группы, где айди группы равен 3
select concat( first_name ,'  ', last_name) as full_name,students.date_of_birth,email, group_name from students
                                                                                                           inner join groups g on g.id = students.group_id where group_id=3;
--7. Вывести все курсы одной группы, где название группы 'Java-13'
select course_name from courses inner join groups on courses.groups_id=groups.id where group_name='Java 13';
--8. Вывести название всех групп и количество студентов в группе
select group_name,count(*)
from students inner join groups g on students.group_id = g.id group by group_name ;
...
select * from students inner join groups g on students.group_id = g.id  where group_id=4 order by first_name desc;
...
select * from lessons join courses c on lessons.course_id = c.id where course_name='Java 9 core';
--3. Вывести количество всех студентов курса id = 4
select students.first_name, count(students.id)from students join groups on students.group_id=group_id join courses
                                                                                                           on groups.id = courses.groups_id where group_id=4  group by students.first_name ;
--4. Вывести имя, почту, специализацию ментора и название курса где курс  айди равен 2
select first_name,email, specialization from mentors join courses c on mentors.course_id = c.id where course_id=2;
--5. Посчитaть сколько менторов в каждом курсе
select count(mentors.id) from mentors join courses c on c.id = mentors.course_id;
...
SELECT courses.course_name, courses.start_date, CONCAT(mentors.first_name, ' ', mentors.last_name) AS mentor_name
FROM courses
         JOIN mentors ON courses.id = mentors.course_id
WHERE courses.start_date BETWEEN '2020-01-01' AND '2023-03-03';
--8. Вывести имя, почту, возраст студентов курса 'Java 13 core'
select students.first_name,students.email, students.date_of_birth,group_name from students
                                                                                      inner join groups g on students.group_id = g.id  join courses c on g.id = c.groups_id where course_name='Java 13 core';

--9. Вывести тот курс  где нет ментора
select courses.course_name, first_name as mentors_name from courses  left join mentors m on courses.id = m.course_id
where course_id is null ;
--10.Вывести тот курс где нет уроков
select courses.course_name, lesson_name from courses left join lessons l on courses.id = l.course_id where lesson_name is null ;
--11.Вывести тот курс  где нет студентов
select courses.course_name from courses left join groups g on
    g.id = courses.groups_id join students s on g.id = s.group_id where first_name is null ;
...
select students.first_name, students.email, students.gender, group_name from students
                                                                                 join groups g on g.id = students.group_id where g.id=2;
--3. Вывести группу студента, айди которого равна 4
select * from students join groups g on g.id = students.group_id where g.id=4;
-- 4. Сгруппируйте студентов по gender и выведите общее количество gender
select count(gender),students.gender from students
group by students.gender;
-- 5. Найдите студента с айди 8 и обновите его данные
update students set first_name='Nurkyz',
                    last_name='Adilbekova',
                    gender='Female',
                    email='adilbekova1234gmail.com',
                    date_of_birth='2000-08-11',
                    group_id=5
where id=8;
--6. Найдите самого старшего студента курса, айди курса которого равна 5
select * from students join groups g on g.id = students.group_id
                       join courses c on g.id = c.groups_id where c.id=5 order by date_of_birth asc limit 1;
--7. Добавьте unique constraint email в столбец таблицы students
alter table students add constraint students_email unique (email);

--8. Добавьте check constraint gender в столбец таблицы mentors
alter table mentors add constraint mentors_gender check ( gender in ('Male', 'Female'));
--9. Добавьте check constraint gender в столбец таблицы students
alter table  students add constraint students_gender check ( gender in ('Male', 'Female') );

--Tasks(Query) Mentors
--1. Вывести средний возраст всех менторов
--mentorlordo age jok eken
SELECT AVG(EXTRACT(YEAR FROM AGE(date_of_birth))) AS average_age
FROM students;
--2. Вывести имя, почту и специализацию ментора группы 'Java-9'
select mentors.first_name, mentors.email, mentors.specialization from mentors
                                                                          join courses c on mentors.course_id = c.id join groups g on g.id = c.groups_id where group_name='Java 9';
...
select * from mentors left join courses c on c.id = mentors.course_id where course_name is null;
-- 5. Вывести айди, имя ментора и его студентов
select mentors.id,mentors.first_name, s.first_name from mentors
                                                            join courses c on mentors.course_id = c.id
...
select concat( m.first_name,' ',  m.last_name) as full_name , count(s.first_name) as student_count
from mentors m
...
select mentors.first_name from mentors
                                   join courses c on c.id = mentors.course_id
                                   join groups g on g.id = c.groups_id
                                   join students s on g.id = s.group_id
where s.first_name is null ;
--8. Вывести ментора у которого студентов больше чем 2
select m.first_name, COUNT(s.id) AS student_count
from mentors m
         join courses c ON c.id = m.course_id
         join groups g ON g.id = c.groups_id
         join students s ON g.id = s.group_id
group by m.first_name having count(s.id)>2;
--9. Вывести курсы ментора с айди 5
select courses.course_name,courses.start_date,m.first_name
from courses
         join mentors m on courses.id = m.course_id
where courses.id=5

--10.Вывести все уроки ментора, айди которого равен 5
select lesson_name , m.first_name
from lessons
         join courses c on lessons.course_id = c.id
...
select lessons.lesson_name,s.first_name from lessons
...
select count(l.id), g.group_name from lessons l
...
select lessons.lesson_name,course_name
from lessons
         join courses c on c.id = lessons.course_id

where c.course_name='Python kids';
--6.Получить все уроки ментора, айди которого равен 5
select lessons.lesson_name, m.first_name from lessons
                                                  join courses c on lessons.course_id = c.id
                                                  join mentors m on c.id = m.course_id
where m.id=5;