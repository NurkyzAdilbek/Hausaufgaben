...
insert into people (id, first_name, last_name, email, gender, age, language) values (86, 'Marylin', 'Fidoe', 'mfidoe2d@reuters.com', 'Female', 26, 'Kyrgyz');
insert into people (id, first_name, last_name, email, gender, age, language)
...
update people set email='testGmail.com' where id=1;
--3. Добавьте новую запись в таблицу
insert into people(id, first_name, last_name, email, gender, age, language)
VALUES  (109,'Nurkyz','Adilbek','adilbekovna345Gmail.com','Female',23,'Kyrgyz');
...
select round(avg(people.age ))from people;
...
--10. Сгруппировать людей по полу и рассчитать средний возраст для каждог пола
...
select count(*) from people where language in ('Kazakh');
...
select count(*)
from people where first_name like 'A%';
--16. Отсартируйте все почты в алфавитном порядке
select people.email from people order by email asc ;
...
select  max(people.language), count(first_name)from people where gender='Female';
...
select * from people where people.gender= 'Male' and  (language in ('Tamil', 'Kazakh', 'Nepali', 'Russian'));
--24. Сгруппируте всех по языку на котором они говорят
select  people.language,COUNT(*) AS count from people group by language;
--25. Отобразите записи фамилии которых содержат только 7 символов и отсортируйте их по айди
select first_name, people.last_name from people where last_name like '_______' order by id;
...
select * from people where age>31 or age<24;
--30. Отобразите уникальные языки
select people.language,count(id) from people group by language having count(id)<=1;
--31. Отобразите записи где в имени или в фамилии есть буква ‘a’ и год рождение с 1969 по 2005 год
select people.first_name, people.last_name from people where last_name ilike 'a' or first_name ilike 'a'
    and (EXTRACT(YEAR FROM age) BETWEEN 1969 AND 2005);
--32. Отобразите айди, имя, почту и возврст каждого человека и отсортируйте по возврасту в убывающем порядке
select id,people.first_name,email, people.age from people  order by age desc;
